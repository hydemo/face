...
export class ZoneService {
  ...
  async partition(parent: string) {
    const partitions: IZone[] = await this.zoneModel
      .find({ parent, buildingType: "61" })
      .sort({ name: 1 });
    await Promise.all(
      partitions.map(async (partition, index) => {
        ...
        await this.zoneModel.update(
          { name },
          { partition: partition._id, partitionSort: index }
        );
  ...
  async createSubZone(
    ...
    let name = buildName
      ? buildName
      : profile.dzqc.replace(parent.profile.dzqc, "");
    ...
    await Promise.all(
      list.map(async (child) => {
        ...
        if (parent.zoneLayer < 1) {
          ...
          subChildren = await this.getChildren(
            subChildren,
            subZone,
            child.dzbm,
            layer,
  ...
  async updateChildren(
    ...
    await Promise.all(
      list.map(async (child) => {
        ...
        let subZone = await this.zoneModel.findOne({
          "profile.dzbm": child.dzbm,
        });
        ...
        if (!subZone) {
          subZone = await this.createSubZone(child, parent);
        }
        children.children.push(String(subZone._id));
        if (parent.zoneLayer < 1) {
          const subList = await this.socUtil.qrcodeAddress(child.dzbm);
          subChildren = await this.updateChildren(
            subChildren,
            subZone,
            subList
          );
  ...
  async updateByQrcode(id: string, code: string) {
    const createParent = await this.zoneModel.findById(id);
    if (!createParent) {
      return;
    }
    const list = await this.socUtil.qrcodeAddress(code);
    const children: IChildren = await this.updateChildren(
      { children: [], hasPartition: false },
      createParent,
      list
    );
    ...
    await this.zoneModel.findByIdAndUpdate(createParent._id, {
      children: children.children,
      hasPartition: children.hasPartition,
    });
  }
  ...
  async addByQrcode(createZone: CreateZoneByScanDTO) {
    ...
    const children: IChildren = await this.getChildren(
      { children: [], hasPartition: false },
      createParent,
      createZone.code,
      createZone.layer,
    ...
    await this.zocUtil.genPropertyCo(
      zip,
      time,
      createParent.propertyCo,
      createParent.detail
    );
    const zocResult: any = await this.zocUtil.upload(zip, time);
    console.log(zocResult, "aa");
    if (zocResult.success) {
      await this.zoneModel.findByIdAndUpdate(createParent._id, {
        isZOCPush: true,
        ZOCZip: zocResult.zipname,
        upTime: Date.now(),
      });
      const client = this.redis.getClient();
  ...
  async findSubZone(parent: string): Promise<IList<IZone>> {
    ...
    const total = await this.zoneModel.countDocuments(condition).exec();
    return { list, total };
  }
  // 获取常住人子集
