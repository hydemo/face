...
import {
  ...
  UpdateZoneDTO
} from "src/module/zone/dto/zone.dto";
...
export class ZoneController {
  ...
  async test(@Request() req, @Query("code") code: string) {
  ...
  async addByQrcode(@Body() zone: CreateZoneByScanDTO) {
  ...
  @Post("/no-qrcode")
  @ApiOperation({ title: "二维码添加小区", description: "二维码添加小区" })
  async addByNoQrcode(@Body() zone: CreateZoneByScanDTO) {
  ...
  // @UserRoles(0)
  @Put("/:id/qrcode")
  @ApiOperation({ title: "二维码更新小区", description: "二维码更新小区" })
  async updateByQrcode(
    @Param("id", new MongodIdPipe()) id: string,
    @Body() zone: UpdateZoneDTO
  ) {
    await this.zoneService.updateByQrcode(id, zone.code);
    return { status: 200, msg: "添加成功" };
  }
  ...
  async getVisitorQrcode(
    ...
    const canActive = await this.roleService.checkRoles({
      user: req.user._id,
      role: 3,
      zone: id,
      isDelete: false
    ...
    const data: string = await this.zoneService.getVisitorQrcode(
      req.user._id,
      id
    );
